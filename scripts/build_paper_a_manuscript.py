...
p = para("")
runs(p, [("就 AI 人才而言，人工智能对就业的结构性影响已有实证——谢萌萌等（2020）发现人工智能对制造业低技能就业的"
          "影响呈非线性特征", False),
         ("[9]", False, (0x18, 0x5F, 0xA5)),
         ("；数字经济发展水平的测度与障碍因子诊断亦有探索", False),
         ("[10]", False, (0x18, 0x5F, 0xA5)),
         ("。但聚焦“AI 人才”的教育供给与产业需求匹配、并下沉到功能链环节的量化诊断仍显不足。", False)])
...
p = para("")
runs(p, [("在教育领域，CCD 亦有应用——蔡文伯等（2022）以耦合协调度测度成渝地区双城经济圈高等教育—科技创新—"
          "经济发展的动态协同", False),
         ("[8]", False, (0x18, 0x5F, 0xA5)),
         ("，为本文将 CCD 用于“教育—产业”场景提供了直接参照。", False)])
p = para("")
runs(p, [("在失调来源的诊断上，障碍度模型常与 CCD 配合使用，通过各指标对协调水平的“偏离贡献”识别关键短板；该方法"
          "已被广泛应用于多领域诊断——如王彦杰等（2022）以熵值法赋权、障碍度模型诊断我国数字经济发展的主要障碍因子", False),
         ("[10]", False, (0x18, 0x5F, 0xA5)),
         ("。", False)])
todo("2.2 可选补强：障碍度模型与熵值法的方法学奠基出处（本文暂以其规范应用文献为引）。")
...
runs(p, [("（4）边界约束的区域组合检验。区域协同不应在任意城市组合上比较，而须先确定“边界”（行政/规划/地理相邻），"
          "再在其同规模的全部组合中看其位置——这是一个置换/组合基准：以全部同规模组合的分布为参照，判断该边界"
          "组合的协同是否仅由组合方式使然（探索性，非显著性检验）；先定边界再比较亦满足可变面积单元问题（MAUP）的要求", False),
...
heading("3.4　识别立场与本文 claim", 2)
para("本文为描述—测度性研究，明确不作因果宣称。原因有二：其一，教育（E）与产业（I）之间存在双向因果——教育因产业"
     "需求而扩张、产业亦向人才富集地集聚——任何单方程估计都难逃联立内生；其二，9 个设区市的样本量不足以支撑可信的"
     "因果识别。故本文克制于“把事实测准、把结构刻清”，将因果识别留待以现代产业学院等政策冲击为基础的后续研究（另文）。")
p = para("")
runs(p, [("据此，本文严格主张的是三类“可被证伪的典型事实”——双核—腹地的二元结构、整体协调所掩盖的链节断裂、区域"
          "协同对“边界”的依赖——而非任何因果效应。相应地，测度类研究的效度威胁不在“内生性”（无待估系数可言内生），而在", False),
         ("构念效度与测量效度", True),
         ("：零值地板、min-max 压缩、可变面积单元问题（MAUP）、口径选择。本文以双轨障碍、熵值法、多种耦合形式与三"
          "口径嵌套加以处理（见第五节），并以全部同规模组合为参照、对“观测到的协同是否仅由组合方式使然”作探索性的"
          "组合基准判断（见 4.5）。凡涉及机制与因果者，本文仅作假设提出、不下断言，并在第六节给出面向后续因果识别的"
          "可检验假设。", False)])
...
para("以全部同规模城市组合为参照，看每个现实边界组合在其中的位置（分位），借此判断观测到的区域协同是否仅由组合"
     "方式使然——这是一个探索性的组合基准，而非显著性检验（图 4）。三种协同类型由此浮现：福州—厦门“跨核互补”的 "
     "D_S 与 BI 均列两市组合之首，结构互补最强（但二者非地理相邻，定位为理论互补对照）；厦漳泉“互补型都市圈”BI 在 "
     "84 个三市组合中居第 6（前 7%），厦门产业领先叠加泉漳教育/承接相对偏强，使并区后供需缺口缩小；福州都市圈、"
     "闽东北“带动型”RSP 在同规模组合中居前 2—5%，由福州高水平拉动，但 BI 中低、互补不强。", before=2)
para("全部组合的分布本身亦给出可证伪的一般规律：含核心市（福/厦）组合的 D_组均值（0.696）约为纯腹地组合（0.292）的 "
     "2.4 倍，纯腹地组合 BI 均值≈0（彼此组团几乎不产生均衡改善，最高 D_组仅 0.352、仍失调）；跨极（同时含教育领先与"
     "产业领先城市）组合 BI 均值为正、不跨极为负。这为“腹地弱联动”提供反事实式证据：均衡改善来自“跨越教育领先↔"
     "产业领先”的镜像配对，腹地需接入核心而非“抱团取暖”。", )
