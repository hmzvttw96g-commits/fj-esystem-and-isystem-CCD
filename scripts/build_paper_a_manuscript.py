...
runs(p, [("（4）边界约束的区域组合检验。区域协同不应在任意城市组合上比较，而须先确定“边界”（行政/规划/地理相邻），"
          "再在其同规模的全部随机组合中比较——这构成一个随机化（置换）检验：以“同规模全部城市组合”为零分布、"
          "以“该边界组合与随机组合无异”为原假设 H₀，其排名分位即置换检验经验 p 值的类比；先定边界再比较亦满足"
          "可变面积单元问题（MAUP）的要求", False),
...
runs(p, [("据此，本文严格主张的是三类“可被证伪的典型事实”——双核—腹地的二元结构、整体协调所掩盖的链节断裂、区域"
         ...
         ("：零值地板、min-max 压缩、可变面积单元问题（MAUP）、口径选择。本文以双轨障碍、熵值法、多种耦合形式与三"
          "口径嵌套加以处理（见第五节），并对“是否超出随机”的判断采用置换检验作形式化推断（见 4.5）。凡涉及机制与"
          "因果者，本文仅作假设提出、不下断言，并在第六节给出面向后续因果识别的可检验假设。", False)])
...
para("把同规模的全部城市组合作为随机化零分布，每个现实边界组合的排名分位即“其协同表现是否超出随机”的经验 p 值"
     "（图 4）。由此三种协同类型获得形式化推断：福州—厦门“跨核互补”的 D_S 与 BI 均列两市组合第 1（经验 p≈0.03），"
     "结构互补最强（但二者非地理相邻，定位为理论互补对照）；厦漳泉“互补型都市圈”BI 居前 7%（经验 p≈0.07，在 10% "
     "水平上拒绝“与随机组合无异”的原假设），厦门产业领先叠加泉漳教育/承接相对偏强，使并区后供需缺口缩小；福州"
     "都市圈、闽东北“带动型”RSP 居同规模前 2—5%（经验 p≈0.05、0.02），由福州高水平拉动，但 BI 中低、互补不强。", before=2)
para("零分布本身亦给出可证伪的一般规律：含核心市（福/厦）组合的 D_组均值（0.696）约为纯腹地组合（0.292）的 2.4 倍，"
     "纯腹地组合 BI 均值≈0（彼此组团几乎不产生均衡改善，最高 D_组仅 0.352、仍失调）；跨极（同时含教育领先与产业"
     "领先城市）组合 BI 均值为正、不跨极为负。这为“腹地弱联动”提供反事实式证据：均衡改善来自“跨越教育领先↔产业"
     "领先”的镜像配对，腹地需接入核心而非“抱团取暖”。", )
para("claim 边界（重要）：置换检验回答的是“观测到的协同是否超出随机组合”，属随机化推断而非因果识别——它排除了"
     "“纯属偶然”，但不排除“选择性聚合”等机制。区域协同的因果效应仍需以政策冲击与跨市人才流动、企业合作数据另文"
     "识别（见第六节可检验假设）。", color=(0x55, 0x55, 0x55), size=10)
