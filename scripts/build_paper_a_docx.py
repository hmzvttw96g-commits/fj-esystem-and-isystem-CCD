...
para("修正落地——双轨障碍诊断：保留全样本 min-max（轨A，衡量“离全省前沿差距”）的同时，剔除福州、厦门后"
     "对腹地 7 市组内重新标准化（轨B，衡量“腹地内部相对短板”）。轨B 消除双核极值压缩，腹地极差大幅放大，"
     "原本“弱信号”的龙岩、三明、南平主障碍变得清晰。两轨一致即诊断稳健。", before=5)
...
para("四环节是否等权？本文以等权为基准（四环节在功能链中地位等同，任一缺失即失调），并以熵值法作稳健性。"
     "熵值法按指标在样本中的离散度赋权，下表为 B 口径全样本熵权。")
table(["环节","熵值法权重","等权"],
