...
para("三市输入：厦门 E=0.713、I=0.977、D=0.913、人口 535；漳州 E=0.126、I=0.033、D=0.254、人口 508；"
     "泉州 E=0.258、I=0.067、D=0.363、人口 891。总人口 P=1934。", before=2, size=9.5)
para("① 区域协调度 D_组 = √(C·T)，其中 C=2√(E_组·I_组)/(E_组+I_组)、T=(E_组+I_组)/2：", before=3, after=1, bold=True, size=9.5)
para("E_组=(535×0.713+508×0.126+891×0.258)/1934 = 0.349；I_组=(535×0.977+508×0.033+891×0.067)/1934 = 0.310；"
     "C=0.998、T=0.330 → D_组=√(0.998×0.330)=0.574。含义：把三市视为一个区域整体时的耦合协调度。", size=9.5)
para("② 区域协同溢价 RSP = D_组 − mean(Dᵢ)：", before=2, after=1, bold=True, size=9.5)
para("mean(Dᵢ)=(0.913+0.254+0.363)/3 = 0.510 → RSP = 0.574−0.510 = +0.063。含义：组团后的协调度，比三市“各自单干"
     "的平均水平”高多少；>0 表示组团优于单市平均。注意：强市配弱市也会把该值抬高，故 RSP 含“带动”成分，非纯互补。", size=9.5)
para("③ 均衡改善度 BI = mean(|Eᵢ−Iᵢ|) − |E_组−I_组|：", before=2, after=1, bold=True, size=9.5)
para("mean|Eᵢ−Iᵢ|=(|0.713−0.977|+|0.126−0.033|+|0.258−0.067|)/3=(0.264+0.092+0.191)/3 = 0.183；"
     "|E_组−I_组|=|0.349−0.310| = 0.039 → BI = 0.183−0.039 = +0.143。含义：三市各自的“教育—产业缺口（|E−I|）”"
