...
heading("6.1  区域指标的计算（人口加权聚合 + 手算示范）", 2)
...
heading("6.2  厦漳泉的检验结果", 2)
para("对 84 组逐一计算上述三指标并各自排序，看厦漳泉组合的名次与分位：", after=2)
...
para("厦漳泉在“整体水平（D_组）”与“简单协同溢价（RSP）”上仅居中游，但在“均衡改善度（BI）”上位列前 6%——"
     "厦门产业领先、泉漳教育与承接相对偏强，三市并区后教育—产业缺口显著缩小，呈现互补型都市圈协同信号。", before=4, size=9.5)
para("结论边界（重要）：BI 高位组合普遍含厦门（前 10% 组合中厦门 8/8），故这是“厦门锚定的互补效应”；厦漳泉因地理邻近"
     "与产业链联系而具备现实都市圈基础、落在高均衡区，但置换检验只能提示协同信号、不能证明因果“抱团”，"
     "机制仍需跨市人才流动、企业合作与产业链数据验证。", before=2, size=9.5)

heading("6.3  全样本一般结论（不限于厦漳泉）", 2)
para("把全部 84 组按“是否含核心市（福/厦）”“是否跨极（同时含教育领先与产业领先城市）”分组，对组内各组合的指标取"
     "“均值”——即该类所有组合的 D_组（或 BI、RSP）的算术平均，反映这一类组合的整体表现。两张对照表导出四条一般结论。")
table(["组合类型","组数","D_组均值","BI均值","RSP均值"],
 [["含核心市(福/厦)","49","0.696","0.029","0.186"],
  ["纯腹地(无核心)","35","0.290","0.003","0.035"]], [2.0,0.9,1.5,1.5,1.5], fontsize=9.5)
para("结论1（核心主导·腹地难自我协同）：含核心组合 D_组均值 0.696 是纯腹地 0.290 的 2.4 倍；纯腹地 BI 均值≈0，"
...
table(["组合类型","组数","BI均值","RSP均值"],
 [["跨极(含教育领先+产业领先)","60","+0.040","0.122"],
  ["不跨极","24","−0.034","0.126"]], [3.2,0.9,1.5,1.5], fontsize=9.5)
para("结论2（镜像即引擎）：跨极组合 BI 均值为正(+0.040)、不跨极为负(−0.034，组团后反而更不均衡)——均衡改善来自"
     "“跨越教育领先↔产业领先”的配对，福州/厦门镜像不只是描述，而是区域协同的物理引擎。两类 RSP 几乎相同"
     "(0.122 vs 0.126) → RSP 不区分互补、BI 才区分。", before=2, size=9.5)
